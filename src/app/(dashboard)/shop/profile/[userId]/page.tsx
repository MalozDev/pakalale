...
export default function ShopProfilePage() {
  ...
  const handleContact = async () => {
    ...
    try {
      ...
      if (existing) {
        router.push(`/shop/chat?chatId=${existing.id}`);
      } else {
        const res = await createChat({ type: "general", participants: [currentUser.id, userId] });
        router.push(res?.chat?.id ? `/shop/chat?chatId=${res.chat.id}` : "/shop/chat");
      }
    } catch (e) {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="px-4 py-4 space-y-4 max-w-2xl mx-auto">
        <Card className="bg-card border-border overflow-hidden">
          <div className="bg-gradient-to-r from-primary/10 via-primary/5 to-amber-golden/10 p-6">
            <div className="flex items-center gap-4">
              <button onClick={() => profile.avatar && openViewer([profile.avatar], 0)} className="shrink-0 cursor-pointer">
                <Avatar className="h-20 w-20 border-2 border-background">
                  <AvatarImage src={profile.avatar} alt={profile.firstName} />
                  <AvatarFallback className="bg-gradient-to-br from-primary to-amber-golden text-primary-foreground text-xl">
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <h1 className="text-xl font-bold truncate">{profile.firstName} {profile.lastName}</h1>
                  {profile.isVerified && <VerifiedBadge size="md" />}
                </div>
                <div className="flex items-center gap-2 mt-1">
                  <Badge variant="secondary" className="text-[10px] capitalize">{isShopOwner ? "Shop Owner" : "Customer"}</Badge>
                  {profile.location && <span className="text-xs text-muted-foreground flex items-center gap-1"><MapPin className="h-3 w-3" />{profile.location}</span>}
                </div>
          ...
          <div className="p-4 space-y-3">
            ...
            <div className="flex gap-4 text-center">
              <div className="flex-1"><p className="text-lg font-bold">{posts.length}</p><p className="text-[10px] text-muted-foreground">Posts</p></div>
              {isShopOwner && shop && (
                <>
                  <div className="flex-1"><p className="text-lg font-bold">{shop.rating || "—"}</p><p className="text-[10px] text-muted-foreground">Rating</p></div>
                  <div className="flex-1"><p className="text-lg font-bold">{shop.totalReviews}</p><p className="text-[10px] text-muted-foreground">Reviews</p></div>
                </>
              )}
            </div>
            {!isCurrentUser && (
        ...
        {isShopOwner && shop && (
          <Card className="bg-card border-border cursor-pointer hover:border-primary/20 transition-colors"
            onClick={() => router.push(`/shop/feed`)}>
        ...
        <div>
          <h3 className="text-sm font-semibold mb-3 flex items-center gap-1.5"><Package className="h-4 w-4 text-primary" /> Posts ({posts.length})</h3>
          {posts.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground"><Package className="h-8 w-8 mx-auto mb-2 opacity-50" /><p className="text-sm">No posts yet</p></div>
          ) : (
      ...
      <ImageViewerModal isOpen={viewerOpen} onClose={() => setViewerOpen(false)} images={viewerImages} initialIndex={viewerIndex} alt="Post image" />
    </div>
  );
}
